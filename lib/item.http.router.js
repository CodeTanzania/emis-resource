...
/**
 * @apiDefine Items
 * @apiSuccess {Item[]} data List of items
...
router.get(PATH_LIST, function getItems(request, response, next) {
  ...
  Item.get(options, function onGetItems(error, results) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.post(PATH_LIST, function postItem(request, response, next) {
  // obtain request body
  const body = _.merge({}, request.body);

  Item.post(body, function onPostItem(error, created) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.get(PATH_SINGLE, function getItem(request, response, next) {
  // obtain request options
  const options = _.merge({}, request.mquery);
  ...
  Item.getById(options, function onGetItem(error, found) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.patch(PATH_SINGLE, function patchItem(request, response, next) {
  // obtain item id
  ...
  Item.patch(id, patches, function onPatchItem(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.put(PATH_SINGLE, function putItem(request, response, next) {
  // obtain item id
  const { id } = request.params;
  ...
  Item.put(id, updates, function onPutItem(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.delete(PATH_SINGLE, function deleteItem(request, response, next) {
  // obtain item id
  const { id } = request.params;

  Item.del(id, function onDeleteItem(error, deleted) {
    // forward error
    if (error) {
      next(error);
    }
