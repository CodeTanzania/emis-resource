...
router.get(PATH_LIST, function getAdjustments(request, response, next) {
  ...
  Adjustment.get(options, function onGetAdjustments(error, results) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.post(PATH_LIST, function postAdjustment(request, response, next) {
  // obtain request body
  const body = _.merge({}, request.body);

  // TODO Stock.adjust(body)
  Adjustment.post(body, function onPostAdjustment(error, created) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.get(PATH_SINGLE, function getAdjustment(request, response, next) {
  // obtain request options
  const options = _.merge({}, request.mquery);
  ...
  Adjustment.getById(options, function onGetAdjustment(error, found) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.patch(PATH_SINGLE, function patchAdjustment(request, response, next) {
  // obtain item id
  ...
  Adjustment.patch(id, patches, function onPatchAdjustment(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.put(PATH_SINGLE, function putAdjustment(request, response, next) {
  // obtain item id
  const { id } = request.params;
  ...
  Adjustment.put(id, updates, function onPutAdjustment(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.delete(PATH_SINGLE, function deleteAdjustment(request, response, next) {
  // obtain item id
  const { id } = request.params;

  // TODO method not supported
  Adjustment.del(id, function onDeleteAdjustment(error, deleted) {
    // forward error
    if (error) {
      next(error);
    }
