...
/**
 * @apiDefine Stocks
 * @apiSuccess {Stock[]} data List of stocks
...
router.get(PATH_LIST, function getStocks(request, response, next) {
  ...
  Stock.get(options, function onGetStocks(error, results) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.post(PATH_LIST, function postStock(request, response, next) {
  // obtain request body
  const body = _.merge({}, request.except('quantity'));

  Stock.post(body, function onPostStock(error, created) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.get(PATH_SINGLE, function getStock(request, response, next) {
  // obtain request options
  const options = _.merge({}, request.mquery);
  ...
  Stock.getById(options, function onGetStock(error, found) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.patch(PATH_SINGLE, function patchStock(request, response, next) {
  // obtain stock id
  ...
  Stock.patch(id, patches, function onPatchStock(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.put(PATH_SINGLE, function putStock(request, response, next) {
  // obtain stock id
  const { id } = request.params;
  ...
  Stock.put(id, updates, function onPutStock(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
 ...
 * @apiUse AuthorizationHeaderErrorExample
 */
router.delete(PATH_SINGLE, function deleteStock(request, response, next) {
  // obtain stock id
  const { id } = request.params;

  Stock.del(id, function onDeleteStock(error, deleted) {
    // forward error
    if (error) {
      next(error);
    }
